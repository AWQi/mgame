		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_READ:
				try {
                    ...
                    if (DataStorage.aX.size()< DataStorage.size){
                        DataStorage.aX.add(fData[0]);
                        DataStorage.aY.add(fData[1]);
                        DataStorage.aZ.add(fData[2]);
                    }else {
                        DataStorage.aX.add(fData[0]);
                        DataStorage.aY.add(fData[1]);
                        DataStorage.aZ.add(fData[2]);
                        DataStorage.aX.remove(0);
                        DataStorage.aY.remove(0);
                        DataStorage.aZ.remove(0);
                    }
					switch (RunMode){
						case 0:
...
//
	...
	public void URLClick(View v) {
		...
		switch (v.getId()){
			case R.id.urlJYZK:intent.setClass(DataMonitor.this,GameActivity.class);break;
			...
			case R.id.showImage:intent.setClass(DataMonitor.this,GameImageShowActivity.class);break;
		}
			...
			case R.id.BtnRecord:
				if (this.recordStartorStop == false)
				{
					...
					new AlertDialog.Builder(this)
							.setTitle("提示")
							.setIcon(android.R.drawable.ic_dialog_alert)
							.setMessage("数据已经记录至手机是否打开已保存的文件？")
							.setPositiveButton("确定", new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface arg0, int arg1) {
									File myFile=new File(PathUtil.getRecordPath()+"Record.txt");
										Intent intent = new Intent(Intent.ACTION_VIEW);
